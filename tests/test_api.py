...
def test_chat_runs_agent_and_persists_messages(monkeypatch):
    ...
    response = client.post("/chat", json={"message": "crie um arquivo", "session_id": "test-session"}, headers=AUTH)
    assert response.status_code == 200
    ...
    assert messages.json()[-2:] == [{"role": "user", "content": "crie um arquivo"}, {"role": "assistant", "content": "Tarefa concluída."}]

def test_agent_requires_api_key(monkeypatch):
    from agent.agent_runner import AgentConfigurationError, run_agent
    monkeypatch.delenv("LLM_API_KEY", raising=False)
    try: run_agent("teste")
    except AgentConfigurationError as exc: assert "LLM_API_KEY" in str(exc)
    else: raise AssertionError("run_agent deveria exigir LLM_API_KEY")
...
def test_destructive_confirmation_guard(monkeypatch):
    ...
    try: run_agent("git push --force origin main")
    except AgentConfigurationError as exc: assert "confirmação humana" in str(exc)
    else: raise AssertionError("ação destrutiva deveria exigir confirmação")
